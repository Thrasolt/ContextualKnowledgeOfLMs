...
class LamaTRExData:
    ...
    def filter_data(self, filter_model: Callable[[Tuple[str, str]], bool]):
        filtered_data: Dict[str, List[Tuple[str, str]]] = {}
        for rel, triples in self.data.items():
            abstract_triples: List[Tuple[str, str]] = list(filter(filter_model, triples))
            if not len(abstract_triples) == 0:
                filtered_data[rel] = abstract_triples
